...
const ServerCard: React.FC<ServerCardProps> = ({ server }) => {
  const handleRetry = (id: number) => {
    fetch(`${API_URL}/${id}/check`, { method: "POST" }).catch((error) =>
  ...
  const cardClasses = `card h-100 ${
    isOffline ? "border-danger offline-card-animation" : ""
  }`;
  const statusTextClasses = `card-text d-flex align-items-center ${
